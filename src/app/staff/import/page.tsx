...
export default function StaffImportPage() {
  ...
  const [activeTab, setActiveTab] = useState<"import" | "leagues" | "teams" | "players" | "fixtures" | "settings">("import");
  ...
  const verifyAuth = async (pw: string, user: string): Promise<boolean> => {
    try {
      const res = await fetch("/api/admin/import", {
        method: "GET",
        headers: { "x-admin-password": pw, "x-staff-username": user },
      });
      return res.ok;
    } catch {
      return false;
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const ok = await verifyAuth(password, username);
      if (ok) {
        try { localStorage.setItem("psafdb_staff_auth", JSON.stringify({ password, username })); } catch {}
  ...
  const handleLogout = () => {
    try { localStorage.removeItem("psafdb_staff_auth"); } catch {}
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("psafdb_staff_auth");
      if (!saved) return;
      const { password: pw, username: user } = JSON.parse(saved);
      if (!pw || !user) return;
      setPassword(pw);
      setUsername(user);
      (async () => {
        if (await verifyAuth(pw, user)) {
  ...
  useEffect(() => {
    if (authenticated && password && username) loadAllData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    return (
      <div className="admin-dark min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <form onSubmit={handleLogin} className="bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-md">
          ...
          <p className="text-gray-400 text-sm mb-6">Match results import</p>
          {loginError && (
            <div className="mb-4 p-3 rounded bg-red-500/20 border border-red-500/30 text-red-200 text-sm">{loginError}</div>
          )}
          <div className="mb-4">
            <label className="block text-gray-300 mb-2">Username</label>
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} autoComplete="username" className="w-full p-3 rounded bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" placeholder="Username" required />
          </div>
          <div className="mb-6">
            <label className="block text-gray-300 mb-2">Password</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" className="w-full p-3 rounded bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" placeholder="Password" required />
          </div>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold text-white mt-2">Match Results Import</h1>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-gray-500 text-sm hidden sm:inline">Signed in as {username}</span>
        ...
        {/* Staff page: match-results import only. No tab bar — other admin
            sections are intentionally unreachable here. */}
